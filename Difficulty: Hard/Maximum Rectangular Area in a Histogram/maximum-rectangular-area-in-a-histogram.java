...
class Solution
{
    ...
    public static long getMaxArea(long hist[], long n) 
    {
        ...
        return max;
    //   long[] rb = new long[(int) n]; // Right boundary
    // Stack<Integer> rbe = new Stack<>();
    // rbe.push((int) n - 1);
    // rb[(int) n - 1] = n;
    
    // for (int i = (int) n - 2; i >= 0; i--) {
    //     while (!rbe.isEmpty() && hist[i] < hist[rbe.peek()]) {
    //         rbe.pop();
    //     }
    //     if (rbe.isEmpty()) {
    //         rb[i] = n;
    ...
    // long[] lb = new long[(int) n]; // Left boundary
    // Stack<Integer> lbe = new Stack<>();
    // lbe.push(0);
    // lb[0] = -1;
    
    // for (int i = 1; i < n; i++) {
    //     while (!lbe.isEmpty() && hist[i] < hist[lbe.peek()]) {
    //         lbe.pop();
    //     }
    //     if (lbe.isEmpty()) {
    //         lb[i] = -1;
